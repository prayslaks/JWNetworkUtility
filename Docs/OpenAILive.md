...
4. 버튼/입력 이벤트에서 **Start Live from Environment**를 호출하고 **Make JWNU Open AI Live Options**를 연결한다.
5. `On Ready` 이후 말한다. 기본값에서는 마이크 입력과 응답 재생이 자동으로 연결된다.
6. 종료 버튼에서 **Stop Live**를 호출한다. `On Closed`까지 기다린다. 다시 시작하려면 다음 Tick/다음 입력에서 Start를 호출한다.
...
`Start Live` / 세션의 `Start`는 런타임에서 직접 제공한 bearer credential도 받는다. `Start ... from Environment`는 기본 공식 주소만 허용하므로 환경변수의 키가 커스텀 테스트 주소로 전송되지 않는다. 일반 키를 배포 클라이언트에 포함하지 않는다. 제품용 클라이언트 인증과 중계 방식은 별도 구성해야 한다.
...
Options.Endpoint를 `ws://127.0.0.1:5000/live/sessions`로 바꾸고 **Start Live**에 **빈 Api Key**를 넣는다. 환경변수 노드를 쓰지 않는다. 서버는 PCM을 에코하고 고정 한국어 자막과 누적 사용량을 보낸다. OpenAI 호출·과금·음성 인식은 없다. 반환된 에코가 다시 마이크로 들어가지 않도록 헤드폰을 사용한다.
...
동일한 BP 경로는 **Create Live Session → 변수 저장 → Bind Event → Start**다. 세션 함수의 false는 접수 실패이며, 시작 설정 실패 시 OnError/OnClosed가 호출 안에서 발생할 수도 있다. 반드시 시작 전에 바인딩한다.
...
| Abort | 즉시 중단. 최종 사용량을 받지 못할 수 있음 |
...
StopLive는 로컬 녹음·재생을 즉시 멈추고 최종 이벤트만 기다린다. 따라서 종료 직전 남은 음성이 잘릴 수 있다. 자연스럽게 다 들은 뒤 종료하려면 UI에서 재생 시간을 고려한다. 월드 정리/EndPlay는 즉시 Abort하며 bFinalized=false일 수 있다.
