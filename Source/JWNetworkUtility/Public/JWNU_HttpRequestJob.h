 ...
 * 클래스 전용 로그 카테고리 선언 
 */
DECLARE_LOG_CATEGORY_EXTERN(LogJWNU_HttpRequestJob, Log, All);

/** 
 * (상태 코드, 네트워크 상태, 리스폰스 바디)를 외부에 전달해주기 위한 델리게이트 선언 
 */
DECLARE_DELEGATE_ThreeParams(FOnHttpRequestJobCompletedDelegate, const int32 /**StatusCode*/, const bool /**bNetworkAvailable*/, const FString& /**ResponseBody*/);
...
struct FJWNU_RequestConfig
{
	 ...
	 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Config")
	bool bRetryOnNetworkError = true;
};
...
#pragma region 설정 및 콜백 필드 모음 : Fields for Configuration and Callback
...
#pragma region 잡 리퀘스트의 초기화-수행-취소를 위한 인터페이스 메서드 모음 : Interface Methods for Initialize, Execute, Cancel Job Requests
...
#pragma region 재시도를 위한 리퀘스트 정보 필드 : Request Info Fields for Retry
...
#pragma region 타임아웃-재시도-취소를 위한 상태 필드 : State Fields for Timeout-Retry-Cancel
...
#pragma region 잡을 컨트롤하기 위한 메서드 : Methods for Control Jobs
	 ...
	 */
	void CompleteJob(const int32 StatusCode, const bool bNetworkAvailable, const FString& ResponseBody);
